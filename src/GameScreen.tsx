...
const GameScreen = ({
  ...
  const gameSpeed = useRef(9);
  const obstacles = useRef<Obstacle[]>([]);
  const collectibles = useRef<Collectible[]>([]);
  const particles = useRef<Particle[]>([]);
  const powerUps = useRef<PowerUp[]>([]);
  const obstacleTimer = useRef(0);
  const nextObstacleGap = useRef(100);
  const currentGapPattern = useRef<"easy" | "medium" | "hard" | "cluster">(
    "easy",
  );
  ...
  const calculateNextGap = () => {
    ...
    const baseGap = Math.max(40, 120 - Math.floor(currentScore / 2000));

    const patterns = {
      easy: { min: 120, max: 180, weight: currentScore < 5000 ? 0.6 : 0.2 },
      medium: { min: 80, max: 120, weight: currentScore < 10000 ? 0.3 : 0.5 },
      hard: { min: 40, max: 80, weight: currentScore < 15000 ? 0.1 : 0.25 },
      cluster: { min: 30, max: 60, weight: 0.05 },
    };

    if (currentGapPattern.current === "cluster" && clusterCount.current > 0) {
      clusterCount.current--;
      return (
        Math.random() * (patterns.cluster.max - patterns.cluster.min) +
        patterns.cluster.min
      );
    }
    ...
    for (const [pattern, config] of Object.entries(patterns)) {
      ...
      if (rand <= cumulativeWeight) {
        currentGapPattern.current = pattern as typeof currentGapPattern.current;

        if (pattern === "cluster") {
          clusterCount.current = 2 + Math.floor(Math.random() * 2);
        }

        const gap = Math.random() * (config.max - config.min) + config.min;
        return Math.max(baseGap * 0.5, gap);
      }
    }
  ...
  const addObstacle = () => {
    ...
    nextObstacleGap.current = calculateNextGap();
  };
  ...
  const startGame = useCallback(() => {
    ...
    nextObstacleGap.current = 100;
    currentGapPattern.current = "easy";
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      gameSpeed.current =
        Math.min(20, 7 + score.current / 5000) * speedBoost.current;
      ...
      // Spawn obstacles with variable gaps
      obstacleTimer.current++;
      if (obstacleTimer.current >= nextObstacleGap.current) {
        addObstacle();
        obstacleTimer.current = 0;
      }
      ...
      context.font = "bold 24px system-ui";
      context.textAlign = "center";
      context.fillText(`Score: ${score.current}`, canvas.width / 2, 35);
      ...
      if (invincible.current) {
        ...
        context.textAlign = "center";
        context.fillText("🛡️ INVINCIBLE", canvas.width / 2, 60);
      }
      ...
      if (speedBoost.current > 1) {
        ...
        context.fillText(
          "⚡ SPEED BOOST",
          canvas.width / 2,
          speedBoost.current > 1 && invincible.current ? 80 : 60,
        );
      }
